...
(function ($, window, undefined) {
	function GWCountdown(opts) {
		if (!(this instanceof GWCountdown)) {
			return new GWCountdown(opts);
		}
		this.options = {};
		this.total   = null;
		this.days    = null;
	...
	GWCountdown.prototype = {
		init: function (opts) {
			...
			self.$container = $(".gw-countdown-container");

			self.$container.on("click", ".gwCountdownSubscribeSelector", function(e){
		...
		setDays: function(){
            ...
            var cdays = $(".gw-days-canvas").get(0);
            ...
            ctx.lineWidth = 3;
            ctx.stroke();
            self.$container.find(".gw-clock-days-container .ss-value").text(self.days);
        },
        setHours: function(){
        	var  self = this;

            var cHr = $(".gw-hours-canvas").get(0);
            ...
            ctx.lineWidth = 3;
            ctx.stroke();
            self.$container.find(".gw-clock-hours-container .gw-value").text(24 - self.hours);
        },
        setMinutes : function(){
        	var  self = this;

            var cMin = $(".gw-minutes-canvas").get(0);
            ...
            ctx.lineWidth = 3;
            ctx.stroke();
            self.$container.find(".gw-clock-minutes-container .gw-value").text(60 - self.minutes);
        },
        setSeconds: function(){
        	var  self = this;

            var cSec = $(".gw-seconds-canvas").get(0);
            ...
            self.$container.find(".gw-clock-seconds-container .gw-value").text(60 - self.seconds);
        },
	...
	return (window.GWCountdown = GWCountdown);
